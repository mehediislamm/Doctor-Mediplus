...
import lottefiels from '../../../../../public/bpzmelYXYK.json'



const AllNeuroDocDetails = () => {
    const [NeuroData, setNeuroData] = useState([]);
    const { category, Neurology_doctor_name } = useParams();
    ...
    const Neuro = NeuroData.filter(data => data.Neurology_doctor_name === Neurology_doctor_name);
    return (
        <div>
            <div>
                {
                    Neuro?.map(OneDate => <div key={OneDate._id}>

                        <div className="max-w-screen-xl mx-auto">
                            <div className="   relative flex justify-center items-center w-full">
                                <img className="w-full h-[500px]" src={OneDate?.Neurology_cover_image} alt="" />
                            </div>

                            <div className="  absolute top-[450px] md:left-72 lg:flex items-center   gap-5">
                                <div className="md:flex justify-center">
                                    <img className="rounded-full border-4 border-t-white w-48 h-48" src={OneDate?.Neurology_doctor_image} alt="" />
                                </div>
                                <div className=" md:text-center lg:mt-10">
                                    <h1 className="font-bold text-xl pt-5 flex justify-center">{OneDate?.Neurology_doctor_name}</h1>
                                    <Rating
                                        emptySymbol={<span style={{ color: "gray", fontSize: "24px" }}>☆</span>}
                                        fullSymbol={<span style={{ color: "gold", fontSize: "24px" }}>★</span>}
                                        initialRating={OneDate?.Neurology_review}
                                        readonly
                                    />
                                    <div className="flex gap-2 items-center">
                                        <FaGraduationCap className="text-[#4073D1] text-xl" />
                                        <h3>{OneDate?.Neurology_graduate}</h3>
                                    </div>
                            ...
                            <div className="  p-5">
                                ...
                                <div className='flex items-center pt-3 pb-5'>
                                    <h1 className='text-4xl text-[#4073D1] font-bold'>{OneDate?.Neurology_price}  </h1>
                                    <p>/per Visit</p>
                                </div>
                                <p>Experienc: {OneDate?.Neurology_exprience}</p>
                                <div className="flex justify-between">
                                    <div>

                                        <h1 className="text-xl font-bold pt-2">Symptoms</h1>

                                        <p className="pt-3">{OneDate?.Neurology_symptoms_of_disease}</p>
                                    </div>
                                    <div>
                                        <TypeAnimation
                                            sequence={[
                                ...
                                <div className="flex items-center gap-2 pt-3">
                                    ...
                                    <p>From {OneDate?.Neurology_location}</p>
                                </div>
...
export default AllNeuroDocDetails;