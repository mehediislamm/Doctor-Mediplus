...
const AllNeuroDoctor = () => {
    ...
    const Neuro = NeuroData?.filter(data => data?.category === 'neurology');
    return (
        <div>
            <div className=" mb-20 max-w-screen-xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                {
                    Neuro?.map(AllNuroData =>
                        <div key={AllNuroData._id} className='  border mt-16 p-5  bg-[#FFFFFF]'>
                            <div className='text-center items-center flex justify-center '>
                                <img className="h-[400px] bg-cover" src={AllNuroData?.Neurology_doctor_image} alt="" />
                            </div>


                            <div>
                                <div className="flex items-center justify-between pt-5">
                                    <h1 className="text-xl font-bold">{AllNuroData?.Neurology_doctor_name}</h1>
                                    <Rating
                                        emptySymbol={<span style={{ color: "gray", fontSize: "24px" }}>☆</span>}
                                        fullSymbol={<span style={{ color: "gold", fontSize: "24px" }}>★</span>}
                                        initialRating={AllNuroData?.Neurology_review}
                                        readonly
                                    />
                                </div>
                                ...
                                <div className="flex justify-between pt-2">
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <h3 className="text-xl">{AllNuroData?.Neurology_graduate}</h3>
                                    </div>
                                </div>

                                <div className="flex justify-between pt-2 pb-5">
                                    <h1>Experience: {AllNuroData?.Neurology_exprience}</h1>
                                    <div className="flex items-center gap-2">
                                        <FaLocationDot className="text-[#4073D1]" />
                                        {AllNuroData?.Neurology_location}
                                    </div>
                                </div>

                                <div>
                                    <Link to={`/allNuroDocDetails/${AllNuroData.Neurology_doctor_name}`}>
