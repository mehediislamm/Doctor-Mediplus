...
const AllGastroDocDetails = () => {
    ...
    const data = GastroData.filter(item => item?.Gastroenterology_doctor_name === Gastroenterology_doctor_name)    
    ...
    return (
        <div>
            <div>
                {
                    data?.map(OneData => <div key={OneData._id}>

                        <div className="max-w-screen-xl mx-auto">
                            <div className="   relative flex justify-center items-center w-full">
                                <img className="w-full h-[500px]" src={OneData?.Gastroenterology_cover_image} alt="" />
                            </div>

                            <div className="  absolute top-[450px] md:left-72 lg:flex items-center   gap-5">
                                <div className="md:flex justify-center">
                                    <img className="rounded-full border-4 border-t-white w-48 h-48" src={OneData?.Gastroenterology_doctor_image} alt="" />
                                </div>
                                <div className=" md:text-center lg:mt-10">
                                    <h1 className="font-bold text-xl pt-5 flex justify-center">{OneData?.Gastroenterology_doctor_name}</h1>
                                    <Rating
                                        emptySymbol={<span style={{ color: "gray", fontSize: "24px" }}>☆</span>}
                                        fullSymbol={<span style={{ color: "gold", fontSize: "24px" }}>★</span>}
                                        initialRating={OneData?.Gastroenterology_review}
                                        readonly
                                    />
                                    <div className="flex gap-2 items-center">
                                        <FaGraduationCap className="text-[#4073D1] text-xl" />
                                        <h3>{OneData?.Gastroenterology_graduate}</h3>
                                    </div>
                            ...
                            <div className="  p-5">
                                ...
                                <div className='flex items-center pt-3 pb-5'>
                                    <h1 className='text-4xl text-[#4073D1] font-bold'>{OneData?.Gastroenterology_price}  </h1>
                                    <p>/per Visit</p>
                                </div>
                                <p>Experienc: {OneData?.Gastroenterology_exprience}</p>
                                <div className="flex justify-between">
                                    <div>

                                        <h1 className="text-xl font-bold pt-2">Symptoms</h1>

                                        <p className="pt-3">{OneData?.Gastroenterology_symptoms_of_disease}</p>
                                    </div>
                                    <div>
                                        <TypeAnimation
                                            sequence={[
                                ...
                                <div className="flex items-center gap-2 pt-3">
                                    ...
                                    <p>From {OneData?.Gastroenterology_location}</p>
                                </div>
