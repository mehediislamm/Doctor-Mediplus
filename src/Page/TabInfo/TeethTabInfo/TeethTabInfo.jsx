...
const TeethTabInfo = ({ Teeth }) => {
    return (
        <div>
            {
                Teeth?.map(TeethData => <div key={TeethData._id}>
                    <div className='grid grid-cols-1 md:grid-cols-1 lg:grid-cols-2 gap-6'>
                        <div>
                            <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2'>
                                <div>
                                    <h2 className='text-xl font-bold'>{TeethData?.Teeth}</h2>
                                    <p className='w-10 h-1 bg-black'></p>

                                    <p className='pt-5'>Experience: <span className='text-[#4073D1]'>{TeethData?.Teeth_exprience}</span></p>
                                    <p className='pt-5'>  <span className='text-[#4073D1] flex items-center gap-2'>
                                        <FaLocationArrow className='text-xl'></FaLocationArrow>
                                        {TeethData?.Teeth_location}
                                    </span></p>
                                </div>
